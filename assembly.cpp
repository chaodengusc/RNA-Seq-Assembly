...
//to get each code of character
unsigned int get2bit[] = {
  (1 << 30) + (1 << 31),
  (1 << 28) + (1 << 29),
  (1 << 26) + (1 << 27),
  (1 << 24) + (1 << 25),
...
//to indicate which read it is from
struct from_read
{
  unsigned int i;
  from_read* next;
...
//int in map<int, read_contain> represent distance between MUPs 
//which support by reads in read_contain
 ...
 * we define MUP which length below mup_lnth0 as complex region.
 * complex region is region with many branches.
 ...
 * here the distance means move steps from one end to other's start,
 * that is to say if two MUPs's distance is one, they are natural connected.
 */
...
void ContigPath_Select(ContigPath_record record, vector<LongMUP*> V,
                       KmerTable &T, ReadTable pV, MUP_Graph &S, LongMUP_Graph &L,
                       Contig_Pathway &w, unsigned int pairkey, int Max, int Min,
                       int Readlen, int Readcov, int paircov)
{
  int k = globalArgs.k;
  int n = globalArgs.n;
  int num = 0;
